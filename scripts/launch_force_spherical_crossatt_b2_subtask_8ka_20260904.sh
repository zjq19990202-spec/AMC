...
SOURCE_SNAPSHOT=${SOURCE_SNAPSHOT:-/mnt/cunchu/zjq/atomic_latent_vla_snapshots/force_spherical_b2_subtask_matched50k_20260904}
RAM_SOURCE=${RAM_SOURCE:-/dev/shm/atomic_latent_vla_run_force_spherical_b2_subtask_20260904}
PYTHON=/mnt/cunchu/zjq/pi0.5_env/.venv/bin/python
RUN=${RUN:-force_spherical45h20_crossatt2_finalzm40_subtask_from_matchedspherical50k_b2s6000_20260904}
...
FORCE_INIT=/mnt/cunchu/zjq/atomic_pi05_force_runs/forceonly_zf_zmforecast_plug_vase_afro50k_w512_b1_s6000_20260831/forceonly_zf_zmforecast_plug_vase_afro50k_w512_b1_s6000_20260831/6000/params
...
OUTER_CONTRACT=${LOG_ROOT}/${RUN}.contract.txt
...
echo "$(date --iso-8601=seconds) corrected-subtask B2 launcher on $(hostname)"
test -x "$PYTHON"
...
test -f "$PLUG_SUBTASK"
test -f "$VASE_SUBTASK"
test -f "$ZM_PARAMS/_METADATA"
test -f "$ZM_PARAMS/manifest.ocdbt"
test -f "$FORCE_INIT/_METADATA"
test -f "$FORCE_INIT/manifest.ocdbt"
...
cat >"$OUTER_CONTRACT" <<EOF
stage=B2 force-conditioned action flow; corrected per-frame subtask prompt
host=$(hostname)
run=$RUN
base_params=$ZM_PARAMS
force_init_params=$FORCE_INIT
datasets=$PLUG_ROOT,$VASE_ROOT
subtask_sidecars=$PLUG_SUBTASK,$VASE_SUBTASK
prompt_route=100_percent_current_subtask_at_anchor; global_prompt_probability=0; atomic_prompt_probability=0
horizon=current_subtask_only_for_all_50_steps; repeat_current_subtask_final_absolute_action_after_boundary; never_concatenate_next_subtask
missing_sidecar_episode_policy=exclude; plug_excluded=36; vase_excluded=29,77
norm=$NORM sha256=$(sha256sum "$NORM" | awk '{print $1}') mode=q01_q99_no_clip adapt_to_pi=true
force_norm=$FORCE_NORM sha256=$(sha256sum "$FORCE_NORM" | awk '{print $1}')
state_clip=off action_clip=off inference_clip=not_applicable optimizer_grad_clip=1.0
zm_route=matched AFRO50K final ordered right/left zM pair; restored 2-head cross-attention in all 18 Action-Expert blocks
visual_geometry=unit-sphere zM inherited from matched spherical AFRO50K; max45deg; 20deg hinge weight0.005
force_memory=30 SlowProj + 10 FastProj tokens per arm; query=Proj(zM)+RTC phase; 2-head cross-attention
force_geometry=512D residual projected to final zM tangent plane; smooth45deg cap; unit-sphere zM_force
trainable=FastProj + full-token force adapter + direct force output + layer gates
frozen=VLM ActionExpert AFRO/Q/codebook completed B1 slow encoder/SlowProj/future decoder
...
schedule=steps6000 warmup300 peak_lr=1e-4 decay_steps6000 decay_lr=1e-5 fresh_adam=true
loss=flow1 future0 delta_regularization1e-4 improvement1 margin0.001 force_rotation0.005
EOF
sha256sum \
...
echo "$(date --iso-8601=seconds) launching one-step corrected-subtask smoke"
"$PYTHON" scripts/train_force_stage_b2.py \
  "${common_args[@]}" \
  --run-name "${RUN}_smoke1" \
  --steps 1 \
  --log-interval 1 \
  --skip-checkpoint \
  >"$SMOKE_LOG" 2>&1
...
if grep -Eq 'Involuntary full rematerialization|RESOURCE_EXHAUSTED|CUDA_ERROR|NCCL.*(error|fail)|out of memory|Traceback' "$SMOKE_LOG"; then
  echo "corrected-subtask B2 smoke violated error gate" >&2
  exit 72
fi
echo "$(date --iso-8601=seconds) corrected-subtask B2 smoke passed"

echo "$(date --iso-8601=seconds) launching formal corrected-subtask B2 6K"
"$PYTHON" scripts/train_force_stage_b2.py \
  "${common_args[@]}" \
  --run-name "$RUN" \
  >"$FORMAL_LOG" 2>&1

test -f "$CHECKPOINT_ROOT/$RUN/$RUN/6000/params/_METADATA"
test -f "$CHECKPOINT_ROOT/$RUN/$RUN/6000/params/manifest.ocdbt"
echo "$(date --iso-8601=seconds) corrected-subtask B2 6000 finalized"
