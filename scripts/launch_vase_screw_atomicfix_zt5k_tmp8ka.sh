...
RUN=union2375_zt25k_vase167_screw213_atomicfix_zt5k_ddp8_bs256_20260825
...
for dataset in "${SCREW}" "${VASE}"; do
  ...
  test -d "${dataset}/meta/fk_horizon_3hz_gate_top5_stay_v2"
done
...

{
  ...
  echo "host=$(hostname)"
  echo "run=${RUN}"
  echo "transition=union2375 ZT25K -> corrected Vase167+Screw213 atomic prompts ZT30K"
  echo "resume=full params+optimizer+global_step from seeded step 25000"
  echo "dataset_1=${SCREW} episodes=213"
  echo "dataset_2=${VASE} episodes=167"
  echo "sampling=concatenated reliable atomic rows with standard shuffled loader"
  echo "prompt_route=corrected atomic_horizon_prompts_3hz.jsonl from both roots"
