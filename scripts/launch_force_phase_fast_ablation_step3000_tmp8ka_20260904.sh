...
RAM_SOURCE=/dev/shm/atomic_latent_vla_eval_phase_fast_step3000_20260904
...
CKPT=${ROOT}/${RUN}/${RUN}/3000
OUT=${ROOT}/evals/${RUN}/step3000_phase_fast_angle_ablation_seed20260824
...
common=(
  ...
  --episode 0 --batch-size 4 --max-windows 16
...
CUDA_VISIBLE_DEVICES=0 "$PYTHON" scripts/evaluate_force_rotation_by_offset.py "${common[@]}" \
  ...
  --output "$OUT/plug_episode0_first16.json" >"$OUT/plug.log" 2>&1 & p0=$!

CUDA_VISIBLE_DEVICES=1 "$PYTHON" scripts/evaluate_force_rotation_by_offset.py "${common[@]}" \
  ...
  --output "$OUT/vase_episode0_first16.json" >"$OUT/vase.log" 2>&1 & p1=$!
