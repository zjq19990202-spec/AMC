...
RAM_SOURCE=/dev/shm/atomic_latent_vla_eval_zf_tcp_alignment_step4000_20260904
...
OUT=${ROOT}/evals/${RUN}/step4000_zf_tcp_increment_alignment_seed20260824
...
common=(
  ...
  --pad-subtask-horizon --skip-wrong-tokens --include-zero-force-update
...
for task in plug vase; do
  task_out="$OUT/${task}_episode0_first16"
  "$PYTHON" scripts/analyze_force_zf_tcp_increment_alignment.py \
    --input "$task_out/decoded_predictions_all_windows.npz" \
    --output "$task_out/zf_tcp_increment_alignment.json" \
    >"$task_out/zf_tcp_increment_alignment.log" 2>&1
done
