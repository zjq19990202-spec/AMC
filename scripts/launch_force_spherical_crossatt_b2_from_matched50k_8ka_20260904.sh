...
SOURCE_SNAPSHOT=${SOURCE_SNAPSHOT:-/mnt/cunchu/zjq/atomic_latent_vla_snapshots/force_spherical45h20_crossatt_b2_matched50k_20260904}
RAM_SOURCE=/dev/shm/atomic_latent_vla_run_force_spherical45h20_crossatt_b2_matched50k_20260904
...
zm_route=matched AFRO50K ordered right/left pair read by restored 2-head atomic_cross_attention in all 18 Action-Expert blocks; layerwise_atomic_flow=false
visual_geometry=unit-sphere zM inherited bitwise from matched spherical AFRO50K; no FiLM substitution
...
audit_line=$(grep 'restored parameter audit' "$SMOKE_LOG" | tail -1)
step_line=$(grep 'step=1 ' "$SMOKE_LOG" | tail -1)
zm_residual_norm=$(sed -n 's/.*zm_residual_proj_norm=\([^ ]*\).*/\1/p' <<<"$audit_line")
adapter_grad=$(sed -n 's/.*adapter_out_grad_norm=\([^ ]*\).*/\1/p' <<<"$step_line")
total_grad=$(sed -n 's/.*grad_norm=\([^ ]*\).*/\1/p' <<<"$step_line")
"$PYTHON" - "$zm_residual_norm" "$adapter_grad" "$total_grad" <<'PY'
import math
import sys
labels = ("zm_residual_proj_norm", "adapter_out_grad_norm", "grad_norm")
values = [float(value) for value in sys.argv[1:]]
for label, value in zip(labels, values, strict=True):
    if not math.isfinite(value) or value <= 1e-10:
        raise SystemExit(f"B2 smoke nonzero gate failed: {label}={value}")
print("B2 smoke nonzero gate:", dict(zip(labels, values, strict=True)))
PY
echo "$(date --iso-8601=seconds) B2 smoke passed with restored AFRO route and live force gradient"
