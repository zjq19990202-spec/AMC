...
RAM_SOURCE=/dev/shm/atomic_latent_vla_eval_force6000_threeway_20260905
...
CKPT=${ROOT}/${RUN}/${RUN}/6000
OUT=${ROOT}/evals/${RUN}/step6000_correct_no_wrong_force_first34_seed20260824
...
common=(
  ...
  --pad-subtask-horizon --include-zero-force-update
  ...
  --episode 0 --batch-size 4 --max-windows 34 --seed 20260824
...
CUDA_VISIBLE_DEVICES=0 "$PYTHON" scripts/evaluate_force_rtc_offsets.py "${common[@]}" \
  ...
  --output-dir "$OUT/plug_episode0_first34" >"$OUT/plug.log" 2>&1 & p0=$!
...
CUDA_VISIBLE_DEVICES=1 "$PYTHON" scripts/evaluate_force_rtc_offsets.py "${common[@]}" \
  ...
  --output-dir "$OUT/vase_episode0_first34" >"$OUT/vase.log" 2>&1 & p1=$!
