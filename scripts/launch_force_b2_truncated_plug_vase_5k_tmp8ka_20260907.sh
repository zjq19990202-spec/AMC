...
SOURCE_SNAPSHOT=${SOURCE_SNAPSHOT:-/mnt/cunchu/zjq/atomic_latent_vla_snapshots/force_b2_truncated_plug_vase_5k_20260907}
RAM_SOURCE=${RAM_SOURCE:-/dev/shm/atomic_latent_vla_run_force_b2_truncated_plug_vase_5k_20260907}
PYTHON=/mnt/cunchu/zjq/pi0.5_env/.venv/bin/python
RUN=${RUN:-force_b2s6000_truncated_plug_vase_unfreeze_ae_zf_rtc5k_20260907}
...
SIDECAR_ROOT=/mnt/cunchu/zjq/force_assets/subtask_sidecars_plug_vase_truncated_adapter_20260907
PLUG_SUBTASK=${SIDECAR_ROOT}/plug_episode_subtasks_through_adapter.jsonl
VASE_SUBTASK=${SIDECAR_ROOT}/vase_episode_subtasks_full.jsonl
...
for value in "${used_mb[@]}"; do
  (( value <= 1000 )) || { echo "GPU is not free; memory_mb=${used_mb[*]}" >&2; exit 71; }
done
...
echo "$(date --iso-8601=seconds) smoke: truncated Plug through adapter + full Vase"
