...
for required in \
  "$PROJECT_ROOT/scripts/start_la4vla_fruit_inference.sh" \
  "$DOWNLOAD_DIR/ar.exp" \
...
launch_terminal "1 LA4VLA 35K Fruit inference" \
  "exec env CUDA_VISIBLE_DEVICES=1 '$PROJECT_ROOT/scripts/start_la4vla_fruit_inference.sh'"

launch_terminal "2 CR1 RoboticsService" \
  "exec expect '$DOWNLOAD_DIR/ar.exp'"

launch_terminal "3 CR1 policy no-RTC20" \
  "echo 'Waiting for LA4VLA server on :12000...'; \
   until ss -ltn | grep -q ':12000 '; do sleep 1; done; \
   echo 'LA4VLA server ready; starting no-force policy bridge'; \
...
echo "Opened four terminals: LA4VLA Fruit, RoboticsService, no-RTC20 policy bridge, rl_deploy ($HOME_POSE pose)."
